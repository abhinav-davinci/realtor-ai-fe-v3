...
function SourceDonut({
  ...
  return (
    <div className="relative mx-auto grid size-[184px] shrink-0 place-items-center">
      ...
      <div className="pointer-events-none absolute inset-0 grid place-items-center text-center">
        <div>
          <p className="text-ink text-[26px] font-bold leading-none tabular-nums">
            {fmt(activeStat ? activeStat.leads : total)}
          </p>
          <p className="text-ink-muted mt-1 text-[11px] font-medium">
            {activeStat ? SOURCE_META[activeStat.source].label : "Total leads"}
          </p>
...
function LegendRow({
  ...
  return (
    <button
      ...
      onBlur={() => onHover(null)}
      className={cn(
        "group flex w-full items-center gap-3 rounded-xl px-2 py-1.5 text-left outline-none transition-all hover:bg-black/[0.02] focus-visible:ring-2 focus-visible:ring-accent-blue/30",
        dim && "opacity-45"
      )}
    >
      <SourceChip source={stat.source} className="size-8" />
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <span className="text-ink truncate text-sm font-semibold">{meta.label}</span>
          {isTop && (
            <span className="bg-brand-orange/10 text-brand-orange rounded-full px-1.5 py-0.5 text-[9px] font-bold tracking-wide uppercase">
              Top
            </span>
          )}
        </div>
        <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-black/[0.05]">
          <div
            ...
            style={{ width: `${(stat.leads / maxLeads) * 100}%`, backgroundColor: meta.color }}
          />
        </div>
      </div>
      <div className="shrink-0 text-right">
        <p className="text-ink text-sm font-bold tabular-nums">{fmt(stat.leads)}</p>
        <p className="text-ink-muted/70 text-[11px] tabular-nums">{pct(stat.share)}</p>
      </div>
    </button>
  );
}
    ...
    <section className="rounded-2xl border border-black/[0.07] bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-ink-muted text-xs">Where your leads come from, and which channel converts best.</p>
        </div>
        <span className="text-ink-muted hidden rounded-full bg-black/[0.04] px-2.5 py-1 text-[11px] font-semibold sm:inline-flex">
          {data.length} channels
        </span>
      </div>

      <div className="mt-5 grid gap-x-6 gap-y-4 lg:grid-cols-[196px_1fr] lg:items-center">
        <SourceDonut data={data} total={summary.total} active={active} onHover={setActive} />
      ...
      <div className="mt-4 flex items-center gap-3 rounded-xl border border-black/[0.06] bg-cream/70 p-3">
        <span className="bg-brand-orange/10 text-brand-orange grid size-9 shrink-0 place-items-center rounded-lg">
          <Trophy className="size-4" />
        </span>
        <p className="text-ink-muted min-w-0 text-xs">
          <span className="text-ink font-semibold">Best converting: {bestMeta.label}.</span>{" "}
          {pct(best.qualifiedRate)} of these leads qualify, the highest of any channel.
        </p>
      </div>
    </section>
  );
