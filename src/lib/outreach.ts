...
/* Contacts now live in the shared master book (src/lib/contacts.ts). The WhatsApp
 * Contacts tab and the broadcast recipient picker read and write that single
 * source of truth, so contacts and lists stay consistent across the app. The real
 * backend contacts service is the org-wide source (see the endpoint note above). */
