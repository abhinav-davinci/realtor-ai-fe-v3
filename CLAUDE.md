...
- **Shared UI:** `components/ai-team/agent-ui.tsx` — `AgentOrb` (aurora-gradient orb; static role icon, or a conversational waveform + breathe when `speaking`; keyframes `agent-aurora-*`/`agent-breathe`/`agent-wave` in globals.css), `ReadinessMeter`, `ChannelBadge`.
